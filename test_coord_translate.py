...
@pytest.mark.parametrize('lat,lon,od',
                         [
                             (0,0,ecco),
                             (45,45,ecco),
                             (50,180,ecco),
...
#     _u.grid2array(od)
    ...
    face,iy,ix = _u.find_ind_h(np.array([lon]),
                              np.array([lat]),
                              tree,
                              h_shape)
    if face is not None:
...
@pytest.mark.parametrize('od',[ospy.open_oceandataset.from_catalog("ECCO"),curv,rect])
def test_rel_4d(x,y,z,t,od):
    _u.grid2array(od)
    it,iz,faces,iys,ixs,rx,ry,rz,rt,cs,sn,dx,dy,dz,dt = _u.find_rel_4d(x,y,z,t)
    assert it.dtype == 'int'
    assert abs(rt).max()<=1
